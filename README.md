...
**Author:** [Gahehe52](https://www.google.com/search?q=https://github.com/gahehe52)
"""
...
readme_content = """# Eksperimen Perbandingan Performa: Baseline (Monolitik) vs Polyglot Persistence

Repositori ini berisi kode sumber untuk riset mengenai efisiensi arsitektur **Polyglot Persistence** (PostgreSQL + MongoDB) dibandingkan dengan arsitektur **Monolitik** (Hanya PostgreSQL) dalam menangani beban kerja analitik *Big Data* menggunakan dataset **OULAD (Open University Learning Analytics Dataset)**.
...
### 1. Kloning Repositori
```bash
git clone [https://github.com/gahehe52/mbd.git](https://github.com/gahehe52/mbd.git)
cd mbd
...
```bash
# Test Baseline (Monolitik)
npx autocannon -c 25 -d 15 http://localhost:3000/api/baseline-heavy/FFF/2013J

# Test Polyglot Persistence
npx autocannon -c 25 -d 15 http://localhost:3000/api/polyglot-heavy/FFF/2013J
...
3. Jalankan sel kode terakhir untuk memulai pengujian otomatis.
4. Skrip akan secara otomatis:
* Menembak endpoint Baseline & Polyglot secara berurutan.
* Menyadap statistik Docker (`docker stats`) selama pengujian.
* Menghasilkan grafik perbandingan *Throughput*, *Latency*, dan *CPU Usage* yang tersinkronisasi.
...
* **Hardware Monitoring**: Fitur monitoring CPU di Jupyter Notebook bergantung pada akses ke perintah `docker stats`. Pastikan Docker Desktop dalam keadaan aktif dan memiliki izin di terminal Anda.
* **Reset Data**: Jika ingin mengosongkan database, gunakan `docker-compose down -v` lalu ulangi langkah persiapan infrastruktur.


Anda bisa langsung menyalin isinya atau mengunduh file Markdown tersebut dan meletakkannya di folder utama proyek repositori GitHub Anda. Penjelasannya sudah mencakup cara pengaturan Docker, proses *indexing* di Prisma, import dataset jutaan baris, hingga cara memunculkan grafik visualisasi CPU dengan Jupyter Notebook.
